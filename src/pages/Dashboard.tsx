...
const Dashboard: React.FC = () => {
  ...
  const fetchComplianceItems = async () => {
    ...
    try {
      ...
      const formattedItems: ComplianceItem[] = items?.map(item => ({
        id: item.id,
        title: item.title,
        description: item.description,
        type: item.type as 'Required' | 'Recommended' | 'Optional',
        status: item.status as 'Completed' | 'Pending',
        industry: item.industry,
        documentUrl: item.document_url || undefined,
        officialSiteUrl: item.official_site_url || undefined,
      })) || [];

      setComplianceItems(formattedItems);
    } catch (error: any) {
                ...
                {isLoading ? (
                  ...
                  <div className="py-12 text-center">
                    ...
                    <p className="mt-2 text-sm text-gray-600">
                      {searchTerm ? 'Try adjusting your search term' : complianceItems.length === 0 ? 'Complete the onboarding process to see your compliance items' : 'No compliance items match the selected filter'}
                    </p>
